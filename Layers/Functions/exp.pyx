import numpy as np
cimport cython
from numba_exp import calc_elem_spec_feat, calc_elem_spec_deriv
...
@cython.boundscheck(False)
@cython.wraparound(False)
def calc_deriv(double[:] kappa_eta, list backward_data, int[:, :] spar_ind, grad_data): # remember to place double[:, :] on grad data
